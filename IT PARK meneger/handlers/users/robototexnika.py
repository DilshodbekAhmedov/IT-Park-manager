...
async def times(call: CallbackQuery, state = FSMContext):
    ...
    send_ex(f"""INSERT INTO db_users_robototexnika (full_name, phone, date, time,language)
...
async def backend(call: CallbackQuery, state = FSMContext):
    info = send_ex("select robot from db_users_courseinfo")
...
async def kompsavodonligi(call: CallbackQuery):
    await call.message.answer("Вы выбрали курс по Робототехнике\nВыберите", reply_markup=course_info_ru)
    await call.message.delete()
